...
func main() {
        ...
        defer watch.Close()
        //添加要监控的对象，文件或文件夹
        err = watch.Add("/root/k8s/commonyaml/")
        checkErr(err)
                        ...
                        case ev := <-watch.Events:
                                {
                                        ...
                                        if ev.Op&fsnotify.Create == fsnotify.Create {
                                                //query all files
                                                files, err := ioutil.ReadDir("/root/k8s/commonyaml/")
                                                checkErr(err)
                                                for _, f := range files {
								...
								if len(dataArray[1]) > 100 {
									log.Println(len(dataArray[0]))
									log.Println("The name of service is too long!")
								} else {
                                                                	if dataArray[2] == "deployment" {
										namespaceID = dataArray[0]
                                                                        	apID := insertApp(dataArray[1])
                                                                        	dpID := insertDeployment(dataArray[1]+"-deployment", apID)
                                                                        	fmt.Println("apID: " + apID)
                                                                        	fmt.Println("dpID: " + dpID)
                                                                        	insertDeploymentTemplate(dataArray[1]+"-deployment", dataArray[3], f.Name(), dpID)
                                                                	}
								}
                                                        }
                                                }
...
func insertDeploymentTemplate(dpName string, dpTag string, dpFile string, dpID string) {
        file, _ := ioutil.ReadFile("/root/k8s/commonyaml/" + dpFile)
	...
	defer db.Close()
        ...
        if dpTpFlag == false {
                ...
                insertDpTp := `insert into wayne.deployment_template  values (` + dpTpID + `, '` + dpName + `', '` + str + `', ` + dpID + `, '` + dpTag + `', '` + currentTime + `', '` + currentTime + `', 'fengxiaodong01', 0);`
                db.Query(insertDpTp)
...
func insertDeployment(name string, apID string) string {
	...
	defer db.Close()
        ...
        if deploymentFlag == false {
                ...
                insertDeploymentName := "insert into wayne.deployment  values (" + deploymentID + ", '" + deploymentName + "', '" + string(deploymentMetaData) + "', "+ apID +", '', 0, '" + currentTime + "', '" + currentTime + "', 'fengxiaodong01', 0);"
...
func insertApp(name string) string {
	...
	defer db.Close()
        ...
        if appFlag == false {
                ...
                currentTime := time.Now().Format("2006-01-02 15:04:05")
                insertAppName := "insert into wayne.app values (" + appID + ", '" + appName + "', " + namespaceID + ", '','"+ appName +"', '" + currentTime + "', '" + currentTime + "', 'fengxiaodong01', 0, 0);"
